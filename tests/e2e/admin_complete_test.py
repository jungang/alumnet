...
def test_login(page):
    ...
    try:
        ...
        log_test("登录跳转", True, f"当前URL: {page.url}")
    except:
        log_test("登录跳转", False, f"当前URL: {page.url}")
        return False
...
def test_alumni_management(page):
    ...
    # 点击校友管理菜单
    try:
        page.click("text=校友管理 >> nth=0")
        page.wait_for_timeout(500)
        page.click("text=校友管理 >> nth=1")
        page.wait_for_load_state("networkidle")
    ...
    # 检查表格列
    columns = ["姓名", "学号", "届别", "班级", "行业", "所在城市"]
    for col in columns:
        try:
            expect(page.locator(f"th:has-text('{col}')")).to_be_visible(timeout=2000)
            log_test(f"表格列-{col}", True)
        except:
            log_test(f"表格列-{col}", False)
...
def test_distinguished_alumni(page):
    ...
    # 检查分类筛选
    try:
        expect(page.locator("text=政界, 商界, 学术, 艺术, 医疗, 教育, 科技").or_(page.locator(".el-select"))).to_be_visible(timeout=3000)
        log_test("分类筛选", True)
    except:
        log_test("分类筛选", False)
...
def test_photo_management(page):
    ...
    # 检查年份筛选
    try:
        expect(page.locator("text=年份").or_(page.locator(".el-select:has-text('年份')"))).to_be_visible(timeout=3000)
        log_test("年份筛选", True)
    except:
        log_test("年份筛选", False)
...
def test_top_scholars(page):
    ...
    for col in columns:
        try:
            expect(page.locator(f"th:has-text('{col}')").or_(page.locator(f"text={col}"))).to_be_visible(timeout=2000)
            log_test(f"表格列-{col}", True)
        except:
            log_test(f"表格列-{col}", False)
...
def test_interaction(page):
    ...
    # 检查审核按钮
    try:
        expect(page.locator("button:has-text('审核')").or_(page.locator("text=审核"))).to_be_visible(timeout=3000)
        log_test("审核功能", True)
    except:
        log_test("审核功能", False)
...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.goto(f"{BASE_URL}/dashboard")
            page.wait_for_load_state("networkidle")
            page.screenshot(path="d:/校友录/tests/e2e/admin_dashboard_final.png", full_page=True)
            print("\n  ✓ 最终截图已保存")
